...
class NewPlan extends Component {
    ...
    componentDidMount(){
        ...
        Axios.post('/api/get_plan_by_id',senderdata).then(res=>{
            let data = res.data;
            this.setState({
                Plantitle:data.title,
                planprice:data.price,
                planlimit:data.limit,
                texteditor:data.discription
            })
        })
    }
    Plantitle(e){
    ...
    onsave(e){
        ...
        if(this.state.Plantitle !='' && this.state.planprice != '' && this.state.planlimit !='' 
            ...
            Axios.post('/api/updateplan',senderdata).then(res=>{
                Swal.fire({
                    position: 'top-end',
                    icon: 'success',
                    ...
                    <div className="card-body  " style={{maxWidth: 'inherit',display:'block',paddingLeft:'10%'}}>
                        ...
                        <div className="col-md-10">
                            <div className=" row col form-group">        
                                ...
                                <input value={this.state.Plantitle || ""} type="name" required    className="form-control col-sm-6 " onChange={this.Plantitle.bind(this)} placeholder="Enter  Plan Title"/>
                            </div>
                        

                            <div className=" row col form-group">        
                                <label className=" mt-2" style={{fontWeight:'bold'}}>Plan Price (month)</label> 
                                <input value={this.state.planprice || ""} type="name" required    className="form-control col-sm-6 " onChange={this.planprice.bind(this)} placeholder="Enter  Plan Price"/>
                            </div>
                        

                            <div className=" row col form-group">        
                                <label className=" mt-2" style={{fontWeight:'bold'}}>Plan Limit (months)</label> 
                                <input  value={this.state.planlimit || ""} type="name" required    className="form-control col-sm-6 " onChange={this.planlimit.bind(this)} placeholder="Enter  Plan Limit"/>
                            </div> 
                            ...
                            <button onClick={this.onsave.bind(this)} className="btn btn-success">Update</button> 
                            </div>
